...
| [`emacs-code-navigator`](emacs-code-navigator/SKILL.md) | 从运行中的 Emacs 获取紧凑且精确的代码上下文，包括未保存的 buffer、项目文件、搜索、Imenu、逐行 xref、Eldoc/Eglot 和 Flymake 诊断。 | “用 Emacs 查看这个符号的定义和引用” |
...
| [`treeland-commit`](treeland-commit/SKILL.md) | 根据实际 diff 生成 Treeland 约定的提交信息，固定包含 `Log`、`PMS` 和 `Influence` 字段，并校验单行字段与行宽。 | “生成 Treeland 格式的 commit message” |

每个目录中的 `SKILL.md` 定义触发条件和工作流；`scripts/*.el` 的公共函数、
docstring 与校验错误构成程序接口。正常使用时无需让助手通读实现源码。
...
4. 只提交本次新建的 Denote 笔记和本次变更的 HyWiki 页面。skill 不会 push。
...
- `treeland-commit`：可加载其 Elisp 脚本的 Emacs；
- `ai-constitution`：无额外运行时依赖。

## Install

将需要的 skill 目录复制或软链接到客户端使用的 skills 目录。仓库结构需要保留；
`denote-scribe` 和 `org-blog-exporter` 都会从同级的 `common/` 加载共享 Git
辅助代码，因此安装其中任意一个时也必须安装 `common/`。
...
zip -r ai-skills.zip \
  ...
  emacs-gtd-assistant org-blog-exporter treeland-commit
```
